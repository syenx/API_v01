    ...
    public class RabbitMQConnection : IRabbitMQConnection
    {
        ...
        public RabbitMQConnection(IConfiguration configuration, ILogger<RabbitMQConnection> logger)
        {
            ...
            _retryPolicy = Policy
                ...
                .WaitAndRetry(5, retryAttempt =>
                {
                    ...
                    _logger.LogWarning($"Conexao com o Rabbit nao realizada, esperando {timeToWait.TotalSeconds} segundos");
                    return timeToWait;
                }
                );
        }
        ...
        public IConnection ConConnect(string connectionName)
        {
            try
            {
                ...
                connection = _retryPolicy.Execute(() => factory.CreateConnection());
        ...
        public (IConnection, IModel) Connect(string connectionName, string queueConfiguration)
        {
            if (connection == null)
            {
                ConConnect(connectionName);
            }
            var channel = QueueDeclareComplete(queueConfiguration);
